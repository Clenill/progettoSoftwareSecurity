...
# Ottiene gli ID utente e gruppo (per impostare i permessi dei file creati)
USER_ID=$(id -u "$(logname)")
GROUP_ID=$(id -g "$(logname)")

# Ottiene chain ID e account di deployment definito nel .env
...
rm -rf blockchain/config/networkFiles blockchain/config/qbftConfigFile.json compose-prod.yml 
...
# Rinomina le directory con le chiavi dei nodi
for i in $(seq 1 4); do
...
# Inserisce gli ID utente e gruppo nel .env per farli usare ai containers
